...
const Pit: React.FC = () => {
  ...
  const handleBuyBonds = useCallback(
    async (amount: string) => {
      ...
      addTransaction(tx, {
        summary: `Buy ${Number(amount).toFixed(2)} Mbond with ${amount} MBird`,
      });
    },
          ...
          <>
            ...
            <StyledBond>
              <StyledCardWrapper>
                <ExchangeCard
                  action="Purchase"
                  fromToken={tombFinance.MBird}
                  fromTokenName="MBird"
                  toToken={tombFinance.MShare}
                  toTokenName="MShare"
                  priceDesc={
                    !isBondPurchasable
                      ? 'MBird is over peg'
                      : `${Math.floor(100 / Number(bondStat.priceInDollars) - 100)}% return when MBird > 1.1POS`
                  }
              ...
              <StyledCardWrapper>
                <ExchangeCard
                  ...
                  fromTokenName="Mbond"
                  toToken={tombFinance.MBird}
                  toTokenName="MBird"
                  priceDesc={`${getDisplayBalance(bondBalance)} Mbond Available`}
                  onExchange={handleRedeemBonds}
                  disabled={!bondStat || bondBalance.eq(0) || !isBondRedeemable}
                  disabledDescription={!isBondRedeemable ? `Enabled when MBird > ${BOND_REDEEM_PRICE}POS` : null}
                />
              </StyledCardWrapper>
            </StyledBond>
          </>
